...
export function useAnnotations(config: UseAnnotationsConfig) {
  ...
  useEffect(() => {
    ...
    fetch(
      `${apiBasePath}/annotations?page=${encodeURIComponent(page)}&context=${encodeURIComponent(context || 'default')}`
    )
      .then((res) => {
  ...
  const updateAnnotation = useCallback(
    async (id: string, updates: Partial<Pick<Annotation, 'x' | 'y' | 'content' | 'imageUrl' | 'closed'>>) => {
      try {
        const response = await fetch(`${apiBasePath}/annotations`, {
          method: 'PUT',
          headers: { 'Content-Type': 'application/json' },
