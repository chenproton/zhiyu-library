...
#### 3.4.1 审批列表页 (`/approval-center`)

**页面入口**：侧边导航 → 通用测评资源管理 → 审批中心

**统计卡片**：审批总数 / 待审批 / 已通过 / 已驳回

**Tab 切换**：题库 / 试卷 / 考试

**搜索与筛选**：
- 搜索框：按标题、提交人搜索
- 状态筛选：全部状态 / 待审批 / 已通过 / 已驳回

**列表字段**（三个 Tab 字段结构相同）：
| 字段 | 说明 |
|------|------|
| 审批类型 | 带图标（BookOpen/GraduationCap/Laptop）|
| 名称 | 题库名称/试卷名称/考试名称 |
| 描述 | 最多两行 |
| 提交人 | |
| 提交时间 | 含时分 |
| 状态 | pending(yellow) / approved(green) / rejected(red) |
| 备注 | |
| 操作 | DropdownMenu |
...
|------|----------|----------|
| 查看详情 | 始终显示 | 打开详情弹窗 |
| 同意 | `status === 'pending'` | 打开同意弹窗，可填备注 |
| 驳回 | `status === 'pending'` | 打开驳回弹窗，可填备注（destructive） |
...
- 标题"审批驳回"
- 备注输入框（非必填）
- **按钮**：取消 / 确认驳回（destructive）

**详情弹窗**：
- 展示审批类型、标题、描述、提交人、提交时间、状态、备注
