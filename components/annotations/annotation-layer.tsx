...
function isInteractiveElement(target: HTMLElement): boolean {
  return !!target.closest(
    'button, input, textarea, select, a, [role="button"], [role="link"], label, [contenteditable="true"]'
  )
}
...
export function AnnotationLayer({
  ...
  // container 模式：等待 containerRef 指向有效 DOM 元素
  useEffect(() => {
  ...
  const calcPosition = useCallback(
    (clientX: number, clientY: number, pageX?: number, pageY?: number): { x: number; y: number } => {
      if (containerRef?.current) {
      ...
      // 全局模式：使用 pageX / pageY 相对于文档
      const px = pageX ?? clientX + window.scrollX
      const py = pageY ?? clientY + window.scrollY
      const w = docSize.width || document.documentElement.scrollWidth || 1
      const h = docSize.height || document.documentElement.scrollHeight || 1
      return {
  ...
  const handleContainerClick = useCallback(
    (e: React.MouseEvent) => {
      ...
      if (target.closest('.ann-dot')) return
      if (target.closest('.ann-panel')) return
      if (target.closest('.ann-editor')) return
      // 智能穿透：不拦截交互元素的点击，避免无法操作弹窗表单
  ...
  const handleDragStart = useCallback(
    (annotationId: string, e: React.MouseEvent | React.TouchEvent) => {
      if (mode !== 'edit') return
      e.stopPropagation()
      setIsDragging(true)
  ...
  useEffect(() => {
    ...
    const handleTouchMove = (e: TouchEvent) => {
      ...
      const pos = calcPosition(touch.clientX, touch.clientY, touch.pageX, touch.pageY)
      setDragPosition(pos)
    }
  ...
  // 层样式：三种模式
  // 1. container 模式：absolute inset-0，挂载到 container 内
  // 2. fixed 模式：fixed inset-0，覆盖 viewport
  // 3. 文档模式：absolute 覆盖文档
  ...
  const layerContent = (
    <>
            ...
            <div
              ...
              style={{
                ...
                boxShadow: isClosed
                  ...
                  : `0 0 0 3px rgba(255,255,255,0.8), 0 0 20px 4px ${primaryColor}80`,
      ...
      {editorPosition && mode === 'edit' && (
        <div
          className="ann-editor pointer-events-none"
